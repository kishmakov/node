...
function createHostProxyHandler(port, itemId, itemStr, proxyStr) {
    ...
    return {
        ...
        get(target, name) {
            ...
            if (name === 'toJSON' && context.getPauseDepth() == 0) {
...
function copyRemainingHandleFields(port, target, source, targetStr) {
    ...
    if (context.isOnHost || context.getPauseDepth() > 0) { // TODO
...
function createWorkerFunctionProxy(port, targetId, memberId, isAsync, funcStr) {
    ...
    return function (...args) {
        const paused = context.getPauseDepth() > 0;
        const method = paused ? 'warm v8' : (isAsync ? 'post' : 'cold v8');
        logToFile(`workerFunctionProxy.1/2 via ${method} ${funcStr}(...)`);
        ...
        if (paused) {
            ...
            logToFile(`RunOnCold before targetId=${targetId} memberId=${memberId}`);
            let value = RunOnCold(context.threadId, context.destId, H2W_CALL, targetId, memberId, args, isAsync);
            logToFile(`RunOnCold after, result=${print(value)}`);
            result = deserializeItem(port, value)
        }
...
function createHostFunctionProxy(port, targetId, memberId, _isAsync, funcStr) {
    ...
    return function (...args) {
        const resultId = context.newId();
        const coord = `targetId=${targetId} memberId=${memberId} resultId=${resultId}`;

        logToFile(`hostFunctionProxy.1/3 ${funcStr}(...) ${coord}`);
        const serArgs = serializeArguments(port, postW2HUpdate, args);

        if (context.getPauseDepth() > 0) {
            logToFile(`hostFunctionProxy.2/3 SyncCall before resultId=${resultId}`);
            const result = SyncCall(context.threadId, targetId, memberId, serArgs, resultId);
            logToFile(`hostFunctionProxy.3/3 SyncCall after, result=${print(result, false)}`);
        ...
        const result = WaitCall(context.threadId, targetId, resultId);
        if (!result) {
...
function dispatchAtHost(reqType, targetId, memberId, rawArgs, isAsync) {
    logToFile(`>>> not implemented for host <<<`);
    return undefined;
}

function dispatchAtWorker(reqType, targetId, memberId, rawArgs, isAsync) {
    if (reqType === H2W_CALL) {
        return callWorkerFunctionPaused(targetId, memberId, rawArgs, isAsync);
    }

    logToFile(`>>> not implemented for ${reqType} <<<`);
    return undefined;
}

function handleRequestOnPaused(reqType, targetId, memberId, rawArgs, isAsync) {
    const dispatch = context.isOnHost ? dispatchAtHost : dispatchAtWorker;
    return dispatch(reqType, targetId, memberId, rawArgs, isAsync);
}
...
function callHostFunction(message) {
    ...
    ResumeCall(context.destId, targetId, resultSer);
    logToFile(`ResumeCall callId=${message.callId} target=${targetId}`);
}
...
function provideType(message) {
    ...
    const targetSer = serializeResult(target, message.id, message.serialize);
    ResumeType(context.destId, message.callId, message.id, targetSer);
    logToFile(`ResumeType callId=${message.callId} id=${message.id}`);
}
...
function setupContext(port, threadId, destId) {
    ...
    RegisterWorker(threadId, handleRequestOnPaused);
    ...
    if (threadId === IDS.HOST) {
        ...
        const { _, resolve, reject } = getAPI(`h:the_promise`);
