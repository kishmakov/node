...
// Shared mutable state moved to internal/worker/state.js
const {
    IDS,
    setThreadId,
    getThreadId,
...
function createItemProxy(port, objectHandle) {
    ...
    const proxies = isHostContext ? workerProxies : hostProxies;
    if (proxies.has(fullId)) return proxies.get(fullId);
    ...
    if (isHostContext && type === EntityType.OBJECT && isAsync) {
        ...
        let base = null;
        if (type === EntityType.FUNCTION) { result = createFunctionProxy(port, itemId, memberId, isAsync, itemStr); Object.assign(result, proxyTag); }
        else if (type === EntityType.CLASS) { base = createFunctionProxy(port, itemId, memberId, isAsync, itemStr); Object.assign(base, proxyTag); }
        else if (type === EntityType.OBJECT) { base = proxyTag; }
