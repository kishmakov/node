...
const {EntityType} = require('internal/worker/serialization_json');
...
function callFunctionAsync(callId, functionId, thisId, valueArgs) {
    ...
    if (context.isOnHost) ResumeCall(context.destId, callId, functionId, valueRes);
}
...
function getTypeCommon(callId, targetId) {
    ...
    const valueRes = serializeResult(callId, objectRes);
    if (IDS.COMM.JSON in valueRes) {
        const json = valueRes[IDS.COMM.JSON];
        if (EntityType.isFunctionType(json[IDS.SER.TYPE])) {
            json[IDS.SER.PARENT] = targetId.replace(/\.[^.]*$/, '');
        }
    }
    return valueRes;
}
