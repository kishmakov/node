...
function createHandle(id, port) {
    ...
    const handler = {
        ...
        get(target, name, receiver) {
            ...
            console.log(`W: Getting.3 '${name}' from h(${id}): before wait`);
            const typeResult = WaitType(id + '.' + name);
            console.log(`W: Getting.4 '${name}' from h(${id}): after wait`);

            let result = {};
            let resultStr = undefined;

            if (typeResult.isFunction) {
                result = createRPC(port, id, name);
                resultStr = 'rpc function';
            } else if ('simpleValue' in typeResult) {
                result = typeResult.simpleValue;
                resultStr = typeResult.simpleValue;
            } else {
                result = createHandle(id + "." + name, port);
                resultStr = 'handle ' + result.HandleID;
            }

            console.log(`W: Getting.5 '${name}' from h(${id}) resulted in ${resultStr}`);
            return result;
        }
    };
...
function createRPC(port, id, name) {
    ...
    return function (...args) {
        ...
        if ('simpleValue' in callResult) {
            result = callResult.simpleValue;
            resultStr = callResult.simpleValue;
        } else {
            result = createHandle(resultId, port);
            resultStr = 'handle ' + result.HandleID;
        }

        console.log(`W: ->${W2H_CALL} h(${id}).${name}(${transferableArgs}) resulted in ${resultStr}`);
        return result;
    };
}
