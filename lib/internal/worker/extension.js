...
function asyncCall(port, targetId, functionId, args, resultId) {
    ...
    logToFile(`W: ->${W2H_CALL} targetId=${targetId} functionId=${functionId} args=${print(args)} ${resultId}`);
...
function serializeFunctionAtWorker(func, providedId = null, targetId = null) {
    ...
    if (!targetId) {
        ...
        logToFile(`W: workerFunctions[${functionId}] := ${print(func)}`);
    }
...
function serializeFunctionAtHost(func, providedId = null, targetId = null) {
    ...
    if (!targetId) {
        ...
        logToFile(`H: hostFunctions[${functionId}] := ${print(func)}`);
    }
    return {NewFunctionID: functionId, NewTargetID: targetId, IsAsync: isProbablyAsync(func)};
}
...
function serializeAtWorker(arg) {
    ...
    if (typeof arg === 'function') result = serializeFunctionAtWorker(arg);
    if (typeof arg === 'object' && arg !== null) result = serializeObjectAtWorker(arg);
...
function serializeAtHost(arg) {
    ...
    if (typeof arg === 'function') result = serializeFunctionAtHost(arg);
    if (typeof arg === 'object' && arg !== null) result = serializeObjectAtHost(arg);
...
function createWorkerObjectProxy(port, objectHandle) {
    ...
    base.map = function() {
        console.warn(`H: >>> map is called, ${objectId}!`);
        return [];
    };
    ...
    const handler = {
        __proto__: null,
        get(target, name) {
            if (name === 'toPrimitive') return () => `proxy[${objectId}]`;
