...
function createWorkerFunctionProxy(port, targetId, memberId, isAsync, funcStr) {
    ...
    return function (...args) {
        ...
        let result
        if (paused) {
            result = v8H2WCall(targetId, memberId, serArgs, isAsync, resultId);
        } else if (isAsync) {
            ...
            let value = RunOnCold(context.threadId, context.destId, callId, H2W_CALL, targetId, memberId, args, isAsync);
...
function createHostFunctionProxy(port, targetId, memberId, _isAsync, funcStr) {
    ...
    return function (...args) {
        ...
        if (context.getPauseDepth() > 0) {
            ...
            const result = RunOnPaused(context.destId, callId, reqType, targetId, memberId, args, isAsync, resultId);
