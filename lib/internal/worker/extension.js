...
function createForEach(port, target, itemId) {
    return function (callback, thisArg) {
        if (target[IDS.COMM.PROTO] === 'Array') {
            const lengthObject = asyncW2HType(port, itemId + '.length', context.newId());
            for (let i = 0; i < lengthObject.simpleValue; i++) {
                const callResult = asyncW2HType(port, `${itemId}.${i}`, context.newId(), true);
...
function createHostProxyHandler(port, itemId, itemStr, proxyStr) {
    ...
    return {
        ...
        get(target, name) {
            ...
            const typeResult = asyncW2HType(port, memberId, context.newId());
...
function createHostFunctionProxy(port, targetId, memberId, isAsync, funcStr) {
    ...
    return function (...args) {
        ...
        if (context.getPaused()) {
            ...
            const result = RunOnPaused(context.destId, callId, reqType, targetId, memberId, args, isAsync, false, resultId);
...
function syncHandler(type, targetId, memberId, jsonArgs, isAsync, serialize) {
    ...
    if (type === W2H_TYPE || type === H2W_TYPE) {
