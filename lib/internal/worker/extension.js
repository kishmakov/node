...
function createHostProxyHandler(port, itemId, itemStr, proxyStr) {
    return {
        ...
        set(target, name, value) {
            const memberStr = `${itemStr}.'${String(name)}'`;
            const serializedValue = serializeArgument(value);
...
function createWorkerFunctionProxy(port, targetId, memberId, isAsync, _funcStr) {
    return function (...args) {
        ...
        if ('simpleValue' in callResult) return callResult.simpleValue;
        if ('type' in callResult) return createItemProxy(port, createHandle(resultId, null, callResult.type, isAsync));
...
function callWorkerFunctionCommon(targetId, memberId, serializedArgs, resultId, isAsync) {
    ...
    if (isAsync && !!result) {
        result
            .then(res => postW2HPromise(context.port, true, resultId, serializeArgument(res)))
            .catch(err => postW2HPromise(context.port, false, resultId, serializeArgument(err)));
    }

    return serializeResult(result, resultId);
}

function callWorkerFunctionPaused(targetId, memberId, rawArgs, resultId, isAsync) {
...
function callHostFunction(message) {
    ...
    try {
        serializedResult = JSON.stringify(serializeArgument(result));
    } catch (e) {
...
function provideType(message) {
    ...
    const serializedTarget = message.serialize ? JSON.stringify(serializeArgument(target)) : '';
