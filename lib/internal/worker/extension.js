    ...
    return {
        ...
        get(target, name) {
            ...
            if (type === EntityType.CLASS || type === EntityType.FUNCTION) {
                ...
                assert(typeResult.code === TypeCode.OBJECT, `W: asyncW2HType returned unknown type ${typeResult.code} for id=${memberId}`);
            }
...
function createWorkerFunctionProxy(port, targetId, memberId, isAsync, _funcStr) {
    return function (...args) {
        ...
        postH2WCall(port, targetId, memberId, serializedArgs, resultId, isAsync);
...
function callWorkerFunction(message) {
    ...
    logToFile(`W: callWorkerFunction resultId=${resultId} ${funcStr}(...)`);
    ...
    try { // try constructor first
        ...
        try { // fall back to regular function call
            ...
            logToFile(`  callWorkerFunction for ${funcStr} failed: ${applyError}\n  func=${func}`);
        }
    }
    ...
    logToFile(`  callWorkerFunction resultId=${resultId} ${funcStr} via ${resultStr} -> ${print(result)}`);
...
function callHostFunction(message) {
    ...
    logToFile(`  callHostFunction for ${funcStr}(...) via ${resultStr} -> ${print(result)} type=${code}`);
...
function provideType(message) {
    ...
    logToFile(`  type=${code} for ${message.id}`);
