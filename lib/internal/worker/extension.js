...
function setupWorkerContext(port, id) {
    ...
    setAPI('w:vscode', createItemProxy(port, createHandle(IDS.VSCODE + '#' + id)));
    setAPI('w:context', createItemProxy(port, createHandle(IDS.CONTEXT + '#' + id)));
    setInterval(() => { logToFile('W: Staying alive'); }, 15000);
}

function setupHostContext(port, id) {
...
function reactOnWorkerMessage(id, message, port) {
    try {
        ...
        logToFile(`Error in ${message} handler: ${e}`);
        throw e;
    }
