...
function callOrConstruct(func, target, resultId, args, funcStr) {
    ...
    logToFile(`  callOrConstruct resultId=${resultId} funcStr=${funcStr} via ${resultStr} -> ${print(result)}`);
    return result;
}

function doCallWorkerFunction(targetId, memberId, serializedArgs, resultId, isAsync) {
    logToFile(`>>> ${typeof serializedArgs}`); // TODO: structure this case
    if (typeof serializedArgs === 'string') serializedArgs = JSON.parse(serializedArgs);
...
function callHostFunction(message) {
    ...
    try {
        serializedResult = JSON.stringify(serializeItem(result));
    } catch (e) {
