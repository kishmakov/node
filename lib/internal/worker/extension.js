...
function callWorkerFunctionSync(callId, targetId, memberId, jsonArgs, isAsync) {
    ...
    return callWorkerFunctionCommon(callId, targetId, memberId, jsonArgs, isAsync);
}

function callWorkerFunctionAsync(callId, targetId, memberId, jsonArgs, isAsync) {
    logToFile(`<-${H2W_CALL} args=${print(jsonArgs)}`);
    callWorkerFunctionCommon(callId, targetId, memberId, jsonArgs, isAsync)
}
...
function callHostFunctionSync(targetId, memberId, jsonArgs) {
    ...
    return callHostFunctionInternal(callId, targetId, memberId, jsonArgs);
}
...
function lookUpTypeCommon(callId, targetId, serialize) {
    const target = context.cache.lookUpItem(targetId, false);
    return serializeResult(target, callId, serialize);
}

function lookUpTypeAsync(threadId, callId, targetId, serialize) {
    const result = lookUpTypeCommon(callId, targetId, serialize);
...
function asyncHandler(message) {
    ...
    if (type === W2H_CALL) callHostFunctionAsync(callId, targetId, memberId, args);
    if (type === H2W_CALL) callWorkerFunctionAsync(callId, targetId, memberId, args, isAsync);
}
