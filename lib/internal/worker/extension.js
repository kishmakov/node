...
function saveObject(object, serialization) {
    ...
    cache.storeItem(serialization.NewItemID, object);
    cache.trackItem(object, serialization);
}
...
function doSerializeItem(arg, targetId = null, memberId = null) {
    const cache = getCurrentCache();
    if (cache.hasTracked(arg)) return cache.getTrackedInfo(arg);
...
function deserializeItem(handle, dstStr = `deserialize(${print(handle)})`) {
    ...
    if (typeof handle === 'object' && handle !== null) {
        if (NewItemID in handle) {
            ...
            result = getCurrentCache().lookUpItem(handle[ReportedItemID]);
...
function callWorkerFunction(targetId, memberId, serializedArgs, resultId, isAsync) {
    let target = getCurrentCache().lookUpItem(targetId);
    ...
    getCurrentCache().storeItem(resultId, result);
}
...
function updateHostObject(targetId, propertyId, value) {
    const target = getCurrentCache().lookUpItem(targetId);
...
function initHost(id, port) {
    if (!hostRuntimes.has(id)) {
        hostRuntimes.set(id, new Runtime(port));

        initLogFile(getThreadId());
        const cache = getCurrentCache();
        const hostId = IDS.VSCODE + '#' + id;
        if (!cache.lookUpItem(hostId, false)) cache.storeItem(hostId, getAPI(`h:vscode.${id}`));
        const contextId = IDS.CONTEXT + '#' + id;
        if (!cache.lookUpItem(contextId, false)) cache.storeItem(contextId, getAPI(`h:context.${id}`));
    }

    return hostRuntimes.get(id);
}

function reactOnWorkerMessage(id, message, port) {
    runtime = initHost(id, port);
    ...
    if (message.type === W2H_TYPE) {
        ...
        const target = getCurrentCache().lookUpItem(message.id, false);
