...
const hostRuntimes = new Map(); // id -> Runtime
...
function saveObject(object, serialization) {
    runtime.cache.storeItem(serialization[IDS.NEW.ITEM], object);
    runtime.cache.trackItem(object, serialization);
}
...
function serializeFunction(func, targetId, memberId) {
    ...
    const result = { [IDS.NEW.ITEM]: targetId, [IDS.NEW.MEMBER]: memberId, [IDS.NEW.TYPE]: type, [IDS.NEW.ASYNC]: isAsync(func) };
...
function serializeObject(object, objectId) {
    ...
    const result = { [IDS.NEW.ITEM]: objectId, [IDS.NEW.MEMBER]: null, [IDS.NEW.TYPE]: EntityType.OBJECT, [IDS.NEW.ASYNC]: null };
...
function doSerializeItem(arg, targetId = null, memberId = null) {
    if (runtime.cache.hasTracked(arg)) return runtime.cache.getTrackedInfo(arg);
...
// Deserialization
function deserializeItem(handle, dstStr = `deserialize(${print(handle)})`) {
    const threadStr = isOnHost() ? 'host' : 'worker';
    let result = handle;
    if (typeof handle === 'object' && handle !== null) {
        if (IDS.NEW.ITEM in handle) {
            result = createItemProxy(handle);
            logToFile(`${dstStr} -> proxies[${handle[IDS.NEW.ITEM]}]`);
        } else if (ReportedItemID in handle) {
            result = runtime.cache.lookUpItem(handle[ReportedItemID]);
...
function createWorkerProxyHandler(itemId, itemStr, proxyStr) {
    return {
        __proto__: null,
        get(target, name) {
...
function createHostProxyHandler(itemId, itemStr, proxyStr) {
    return {
        __proto__: null,
        get(target, name) {
            if (name in target) return target[name];
            ...
            const typeResult = asyncType(runtime.port, memberId);
            ...
            let handle = {[IDS.NEW.ITEM]: newItemId, [IDS.NEW.MEMBER]: newMemberId, [IDS.NEW.TYPE]: type}
            target[name] = createItemProxy(handle);
...
function copyRemainingHandleFields(target, source, targetStr) {
    for (const memberId of Reflect.ownKeys(source)) {
        target[memberId] = deserializeItem(source[memberId], `${targetStr}.'${String(memberId)}'`);
    }
    if (source.PrototypeID === 'Array') Object.setPrototypeOf(target, Array.prototype);
}

function createItemProxy(objectHandle) {
    ...
    if (runtime.cache.hasProxy(fullId)) return runtime.cache.getProxy(fullId);
    ...
    if (isHostContext && type === EntityType.OBJECT && isAsync) {
        ...
        if (type === EntityType.FUNCTION) {
            result = createFunctionProxy(itemId, memberId, isAsync, itemStr);
            Object.assign(result, createProxyTag(fullId));
        } else if (type === EntityType.CLASS) {
            base = createFunctionProxy(itemId, memberId, isAsync, itemStr);
        ...
        if (base) result = new Proxy(base, createProxyHandler(fullId, itemStr, proxyStr));
        runtime.cache.storeProxy(fullId, result);
        if (base) copyRemainingHandleFields(base, remainingHandle, itemStr);
    }
    return result;
}

function createWorkerFunctionProxy(targetId, memberId, isAsync, funcStr) {
    funcStr = (isAsync ? 'async ' : '') + funcStr;
    return function (...args) {
        const serializedArgs = args.map(arg => serializeItem(arg));
        const resultId = newId();
        runtime.port.postMessage({ type: H2W_CALL, targetId, memberId, args: serializedArgs, resultId, isAsync });
        logToFile(`H: ->${H2W_CALL} ${funcStr}(${print(serializedArgs)}) resultId=${resultId}`);
        return createItemProxy({ [IDS.NEW.ITEM]: resultId, [IDS.NEW.TYPE]: EntityType.OBJECT, [IDS.NEW.ASYNC]: isAsync });
    };
}

function createHostFunctionProxy(targetId, memberId, _isAsync, funcStr) {
    ...
    function hostFuncProxy(...args) {
        ...
        const callResult = asyncCall(runtime.port, targetId, memberId, serializedArgs, resultId);
        logToFile(`W: call.2 ${funcStr}(${argsStr}) callResult=${print(callResult)}`);

        let result = 'simpleValue' in callResult
            ? callResult.simpleValue
            : createItemProxy({[IDS.NEW.ITEM]: resultId, [IDS.NEW.TYPE]: callResult.type});
...
function callWorkerFunction(targetId, memberId, serializedArgs, resultId, isAsync) {
    let target = runtime.cache.lookUpItem(targetId);
    ...
    const args = serializedArgs.map(arg => deserializeItem(arg));
    ...
    runtime.cache.storeItem(resultId, result);
}

function callHostFunction(targetId, memberId, serializedArgs, resultId, fromThreadId) {
    let target = runtime.cache.lookUpItem(targetId);
    ...
    if (!ResumeCall(resultId, fromThreadId, result, code)) {
        runtime.cache.storeItem(resultId, result);
    }
}

// Object property update management
function updateHostObject(targetId, propertyId, value) {
    const target = runtime.cache.lookUpItem(targetId);
    target[propertyId] = deserializeItem(value);
...
function initWorker(id, port) {
    ...
    setAPI('w:vscode', createItemProxy({ [IDS.NEW.ITEM]: IDS.VSCODE + '#' + id, [IDS.NEW.TYPE]: EntityType.OBJECT }));
    setAPI('w:context', createItemProxy({ [IDS.NEW.ITEM]: IDS.CONTEXT + '#' + id, [IDS.NEW.TYPE]: EntityType.OBJECT }));
    setInterval(() => { logToFile('W: Staying alive'); }, 15000);
}

function initHost(id, port) {
    if (!hostRuntimes.has(id)) {
        runtime = new Runtime(port);
        hostRuntimes.set(id, runtime);

        initLogFile(getThreadId());
        const hostId = IDS.VSCODE + '#' + id;
        if (!runtime.cache.lookUpItem(hostId, false)) runtime.cache.storeItem(hostId, getAPI(`h:vscode.${id}`));
        const contextId = IDS.CONTEXT + '#' + id;
        if (!runtime.cache.lookUpItem(contextId, false)) runtime.cache.storeItem(contextId, getAPI(`h:context.${id}`));
    } else {
        runtime = hostRuntimes.get(id);
    }
}
...
function reactOnWorkerMessage(id, message, port) {
    ...
    if (message.type === W2H_CALL) {
        ...
        callHostFunction(message.targetId, message.memberId, message.args, message.resultId, message.threadId);
        return true;
    }

    if (message.type === W2H_SET) {
        logToFile(`H: <-${W2H_SET}`);
        updateHostObject(message.targetId, message.propertyId, message.value);
        return true;
    }
    ...
    if (message.type === W2H_PROMISE) {
        ...
        func(deserializeItem(message.result));
        return true;
    }

    if (message.type === W2H_TYPE) {
        logToFile(`H: <-${W2H_TYPE} id=${message.id} callId=${message.callId}`);
        const target = runtime.cache.lookUpItem(message.id, false);
