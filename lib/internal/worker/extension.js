...
function initWorker(id, port) {
    ...
    setAPI('w:vscode', createItemProxy(port, {NewItemID: IDS.VSCODE, NewType: EntityType.OBJECT}));
...
function initHost(id) {
    if (hostItems.size === 0) hostItems.set(IDS.VSCODE, getAPI('h:vscode'));
    hostItems.set(IDS.CONTEXT + '#' + id, getAPI(`h:context.${id}`));
    console.log(`H: Initialized host with id=${id}`);
}
