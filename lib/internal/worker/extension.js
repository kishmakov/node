...
function createWorkerFunctionProxy(port, targetId, memberId, isAsync, funcStr) {
    ...
    return function (...args) {
        ...
        const result = anyH2WCall(port, targetId, memberId, isAsync, args);
...
function createHostFunctionProxy(port, targetId, memberId, isAsync, funcStr) {
    ...
    return function (...args) {
        ...
        const result = anyW2HCall(port, targetId, memberId, isAsync, args);
...
function callOrConstruct(func, target, resultId, args, funcStr) {
    ...
    try { // try constructor first
        ...
        try { // fall back to regular function call
            ...
            logToFile(`  callOrConstruct failed funcStr=${funcStr}\n`);
            logToFile(`    target=${target}`);
            logToFile(`    args=${args}`);
            logToFile(`    msg=${applyError.message}`);
            logToFile(`    stack=${applyError.stack}\n`)
            logToFile(`    func=${func}`, true);
        }
    }
...
function callWorkerFunctionCommon(targetId, memberId, jsonArgs, resultId, isAsync) {
...
function callWorkerFunctionSync(callId, targetId, memberId, jsonArgs, isAsync) {
    ...
    return callWorkerFunctionCommon(targetId, memberId, jsonArgs, resultId, isAsync);
}

function callWorkerFunctionAsync(targetId, memberId, jsonArgs, resultId, isAsync) {
    logToFile(`<-${H2W_CALL} args=${print(jsonArgs)}`);
    callWorkerFunctionCommon(targetId, memberId, jsonArgs, resultId, isAsync)
}

function callHostFunctionInternal(targetId, memberId, jsonArgs, resultId) {
...
function callHostFunctionSync(targetId, memberId, jsonArgs) {
    assert(typeof jsonArgs === 'string', 'failed: jsonArgs != string @ callHostFunctionSync');
    const resultId = context.newId();
    return callHostFunctionInternal(targetId, memberId, jsonArgs, resultId);
}

function callHostFunctionAsync(callId, targetId, memberId, jsonArgs, resultId) {
    logToFile(`<-${W2H_CALL} args=${print(jsonArgs)}`);
    const resultSer = callHostFunctionInternal(targetId, memberId, jsonArgs, resultId);
