...
function createWorkerFunctionProxy(port, targetId, memberId, isAsync, funcStr) {
    return function (...args) {
        ...
        logToFile(`calling via ${paused ? 'v8' : 'post'} ${funcStr}`);
        const serArgs = paused ? serializeArgumentsV8(args) : args.map(serializeArgumentPost);
        const result = paused
            ? v8H2WCall(targetId, memberId, serArgs, isAsync)
            : postH2WCall(port, targetId, memberId, serArgs, isAsync);
...
function createHostFunctionProxy(port, targetId, memberId, _isAsync, funcStr) {
    ...
    return function (...args) {
        const serializedArgs = args.map(serializeArgumentPost);
        const resultId = context.newId();
        logToFile(`W: call.1 ${funcStr}(...), reserving resultId=${resultId}`);
        const callResult = asyncW2HCall(port, targetId, memberId, serializedArgs, resultId);
        logToFile(`W: call.2 ${funcStr}(...) callResult=${print(callResult)}`);
        ...
        logToFile(`W: call.3 ${funcStr}(...) resulted in ${result}`);
        return result;
    };
}
