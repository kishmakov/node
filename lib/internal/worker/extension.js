...
function deserializeResultPost(port, value) {
    logToFile(`deserializeResultPost value=${print(value)}`);
    const handle = createHandle(value[IDS.COMM.RESULT], null, EntityType.OBJECT, value.isAsync);
    return createItemProxy(port, handle);
}

function deserializeResultV8(port, value) {
    logToFile(`deserializeResultV8 value=${print(value)}`);
    ...
    // TODO: reported or new item
    mixInType(value);
    const handle = createHandle(value[IDS.COMM.RESULT], null, value.type, value.isAsync);
    return createItemProxy(port, handle);
}
...
function createWorkerFunctionProxy(port, targetId, memberId, isAsync, funcStr) {
    return function (...args) {
        if (context.isThreadPaused()) {
            logToFile(`calling via v8 ${funcStr}`);
            const serArgs = serializeArgumentsV8(args);
            return deserializeResultV8(port, v8H2WCall(targetId, memberId, serArgs, isAsync));
        }

        logToFile(`calling via post ${funcStr}`);
        const serArgs = args.map(serializeArgumentPost);
        return deserializeResultPost(port, postH2WCall(port, targetId, memberId, serArgs, isAsync));
    };
}
...
function createHostFunctionProxy(port, targetId, memberId, _isAsync, funcStr) {
    ...
    return function (...args) {
        const serializedArgs = args.map(arg => serializeArgumentPost(arg));
        ...
        let handle = createHandle(resultId, null, callResult.type);
        if (!!callResult._as_json) Object.assign(handle, callResult._as_json);

        let result = 'simpleValue' in callResult
            ? callResult.simpleValue
            : createItemProxy(port, handle);
