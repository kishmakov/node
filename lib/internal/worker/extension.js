...
function createHostFunctionProxy(port, targetId, memberId, _isAsync, funcStr) {
    ...
    return function (...args) {
        const resultId = context.newId();
        const coord = `targetId=${targetId} memberId=${memberId} resultId=${resultId} depth=${context.getPauseDepth()}`;
        ...
        const callId = postW2HCall(port, targetId, memberId, serArgs, resultId);

        logToFile(`hostFunctionProxy.2/3 WaitCall before resultId=${resultId}`);
        const result = WaitCall(context.threadId, callId, targetId, resultId);
        if (!result) {
...
function callWorkerFunctionSync(targetId, memberId, jsonArgs, isAsync) {
    assert(typeof jsonArgs === 'string', 'args must be a string');
    const resultId = context.newId();
...
function callHostFunctionCommon(callId, targetId, memberId, serializedArgs, resultId) {
    ...
    logToFile(`ResumeCall callId=${callId} target=${targetId}`);
}
...
function callHostFunctionAsync(callId, targetId, memberId, serializedArgs, resultId) {
    ...
    callHostFunctionCommon(callId, targetId, memberId, serializedArgs, resultId)
}
...
function asyncHandler(message) {
    ...
    if (type === W2H_TYPE || type === H2W_TYPE) lookUpTypeAsync(threadId, callId, targetId, serialize);
...
function syncHandler(type, targetId, memberId, jsonArgs, isAsync) {
    if (!context.isOnHost && type === H2W_CALL) {
        return callWorkerFunctionSync(targetId, memberId, jsonArgs, isAsync);
    }

    logToFile(`>>> not implemented for ${type} <<<`);
    return undefined;
}
