...
function deserializeItem(port, handle, dstStr = `deserialize(${print(handle)})`) {
    ...
    if (typeof handle === 'object' && handle !== null) {
        if (IDS.NEW.ITEM in handle) {
            ...
            logToFile(`  ${dstStr} looked up as items@${context.cache.name}[${handle[IDS.REPORTED]}]`);
        }
    } else {
...
function callHostFunction(message) {
    ...
    const argsStr = print(serializedArgs);
    logToFile(`<-${W2H_CALL} args=${argsStr}`);
    ...
    logToFile(`  callHostFunction for ${funcStr}(${argsStr}) via ${resultStr} -> ${print(result)} type=${code}`);
