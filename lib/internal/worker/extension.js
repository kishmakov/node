...
function setupContext(port, threadId, destId) {
    ...
    context = new Context(port, threadId);
...
const handlers = {
    ...
    // host
    [W2H_CALL](message) {
        callHostFunction(context.port, message.targetId, message.memberId, message.args, message.resultId, message.threadId);
    },
    [W2H_SET](message) {
        updateHostObject(context.port, message.targetId, message.propertyId, message.value);
    },
    [W2H_PROMISE](message) {
