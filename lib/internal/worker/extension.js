...
const hostContexts = new Map(); // id -> Runtime
...
function doSerializeItem(arg, targetId = null, memberId = null) {
    if (context.cache.hasTracked(arg)) return context.cache.getTrackedInfo(arg);
...
function deserializeItem(handle, dstStr = `deserialize(${print(handle)})`) {
    ...
    if (typeof handle === 'object' && handle !== null) {
        if (IDS.NEW.ITEM in handle) {
            ...
            result = context.cache.lookUpItem(handle[ReportedItemID]);
...
function createHostProxyHandler(itemId, itemStr, proxyStr) {
    return {
        ...
        get(target, name) {
            ...
            const typeResult = asyncType(context.port, memberId);
...
function createWorkerFunctionProxy(targetId, memberId, isAsync, funcStr) {
    ...
    return function (...args) {
        ...
        context.port.postMessage({ type: H2W_CALL, targetId, memberId, args: serializedArgs, resultId, isAsync });
...
function createHostFunctionProxy(targetId, memberId, _isAsync, funcStr) {
    ...
    function hostFuncProxy(...args) {
        ...
        const callResult = asyncCall(context.port, targetId, memberId, serializedArgs, resultId);
...
function initWorker(id, port) {
    ...
    context = new Context(port, getCurrentCache());
...
function initHost(id, port) {
    if (!hostContexts.has(id)) {
        context = new Context(port, getCurrentCache());
        hostContexts.set(id, context);

        initLogFile(getThreadId());
        const hostId = IDS.VSCODE + '#' + id;
        if (!context.cache.lookUpItem(hostId, false)) context.cache.storeItem(hostId, getAPI(`h:vscode.${id}`));
        const contextId = IDS.CONTEXT + '#' + id;
        if (!context.cache.lookUpItem(contextId, false)) context.cache.storeItem(contextId, getAPI(`h:context.${id}`));
    } else {
        context = hostContexts.get(id);
    }
}
