...
function createHostFunctionProxy(port, targetId, memberId, _isAsync, funcStr) {
    ...
    return function (...args) {
        ...
        logToFile(`call.1/3 ${funcStr}(...)`);
        const resultId = context.newId();
        postW2HCall(port, targetId, memberId, serializedArgs, resultId);

        assert(!context.isThreadPaused(), 'W: failed to call createHostFunctionProxy on non-paused thread');

        logToFile(`call.2/3 WaitCall before resultId=${resultId}`);
        const result = WaitCall(context.threadId, resultId);
        logToFile(`call.3/3 WaitCall after, result=${print(result)}`);
