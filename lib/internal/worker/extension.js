...
function createForEach(port, target, itemId) {
    return function (callback, thisArg) {
        if (target[IDS.COMM.PROTO] === 'Array') {
            const lengthObject = asyncW2HType(port, itemId + '.length', newCallId());
            for (let i = 0; i < lengthObject.simpleValue; i++) {
                const callResult = asyncW2HType(port, `${itemId}.${i}`, newCallId(), true);
...
function createHostProxyHandler(port, itemId, itemStr, proxyStr) {
    ...
    return {
        ...
        get(target, name) {
            ...
            const typeResult = asyncW2HType(port, memberId, newCallId());
...
function createHostFunctionProxy(port, targetId, memberId, _isAsync, funcStr) {
    ...
    return function (...args) {
        ...
        const coord = `callId=${callId} targetId=${targetId} memberId=${memberId} resultId=${resultId} depth=${context.getPauseDepth()}`;
        ...
        if (context.getPauseDepth() > 0) {
            ...
            const result = SyncCall(context.threadId, callId, targetId, memberId, serArgs, resultId);
