...
function deserializeAt(port, arg) {
    ...
    if (typeof arg === 'object' && arg !== null) {
        if (NewObjectID in arg) {
            ...
            logToFile(`${dst} -> ${proxyStr}[${arg[NewObjectID]}]`);
        }
        if (NewFunctionID in arg) {
            result = createFunctionProxy(port, arg);
            logToFile(`${dst} -> method ${proxyStr}[${arg[NewFunctionID]}]`);
        }
        if (OldObjectID in arg) {
            result = lookUpObject(arg[OldObjectID], true);
            logToFile(`${dst} -> ${threadStr}Objects[${arg[OldObjectID]}]`);
        }
        if (OldFunctionID in arg) {
            result = lookUpFunction(arg[OldFunctionID], true);
            logToFile(`${dst} -> ${threadStr}Functions[${arg[OldFunctionID]}]`);
        }
    } else {
        logToFile(`${dst} -> ${print(result)}`);
    }
    ...
    for (const memberId of Reflect.ownKeys(remainingHandle)) {
        ...
        if (typeof val === 'object' && val !== null) {
            ...
            if (OldObjectID in val) {
                base[memberId] = lookUpObject(val[OldObjectID], true);
