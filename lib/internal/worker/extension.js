...
function logToFile(message, printStack = false) {
    ...
    const logPath = path.join(process.env.ISOLATION_LOG_DIR, `log_${threadId}.txt`);
...
function asyncCall(port, targetId, memberId, args, resultId) {
    ...
    const callResult = WaitCall(resultId, threadId);
        ...
        return function(callback, thisArg) {
            if (Array.isArray(target)) {
                ...
                Object.keys(target).forEach(key => {
                    callback.call(thisArg, target[key], key, target);
                });
            }
        };
    }
...
function createHostProxyHandler(port, itemId, itemStr, proxyStr) {
    return {
        ...
        get(target, name) {
            ...
            if (value) {
                target[name] = value;
                logToFile(`W: get ${memberStr} resulted via hardcoded in ${print(value)}`);
                return value;
            }
...
function initWorker(id, port) {
    threadId = id.replace(/\./g, '').replace(/[^a-zA-Z0-9]/g, '_').toLowerCase();
    workerPort = port;
