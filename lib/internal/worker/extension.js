...
function serializeObjectAtHost(arg, providedId = null, seen = new WeakMap()) {
    ...
    logToFile(`H: hostObjects[${id}] := ${arg?.constructor?.name ?? print(arg)}`); // TODO: is it needed?
...
function deserializeAtHost(port, arg) {
    ...
    if (typeof arg === 'object' && arg !== null) {
        if (NewObjectID in arg)  return createWorkerObjectProxy(port, arg);
        if (NewFunctionID in arg) return createWorkerFunctionProxy(port, null, arg.NewFunctionID, arg.IsAsync);
        if (OldObjectID in arg)  return hostObjects.get(OldObjectID);
        if (OldFunctionID in arg)  return hostFunctions.get(OldFunctionID);
    }
...
function deserializeAtWorker(port, arg) {
    ...
    if (typeof arg === 'object') {
        if (NewObjectID in arg) return createHostObjectProxy(port, arg[NewObjectID]);
        if (NewFunctionID in arg) return createHostFunctionProxy(port, arg[NewFunctionID]);
    }
        ...
        get(target, name) {
            ...
            if (typeof val === 'object') {
                ...
                if (NewFunctionID in val) {
                    target[name] = createWorkerFunctionProxy(port, val[NewTargetID], val[NewFunctionID], val[IsAsync]);
                    return target[name];
                }
            }
