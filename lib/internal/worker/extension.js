...
hostObjects.set = function(key, value) {
    const repr = Object.values(IDS).includes(key) ? key.toUpperCase() : print(value);
    logToFile(`H: hostObjects[${key}] := ${repr}`);
...
function serializeFunction(func, providedId = null, targetId = null) {
    ...
    return {NewFunctionID: functionId, NewTargetID: targetId, IsAsync: isProbablyAsync(func), IsClass: isClass(func)};
}

function serializeObject(arg, providedId = null) {
    if (arg === null || typeof arg !== 'object') return arg;

    const objectsTracker = isOnHost() ? hostObjectsTracker : workerObjectsTracker;
    if (objectsTracker.has(arg)) return {NewObjectID: objectsTracker.get(arg)};
    ...
    while (current && current !== Object.prototype) {
        ...
        for (const memberId of Reflect.ownKeys(current)) {
            ...
            if (typeof val === 'function') {
                ...
                result[memberId] = serializeObject(val, id + '.' + String(memberId));
            } else {
                result[memberId] = val;
            }
        }
...
function commonFieldGetter(name, objectId, nameStr) {
...
function objectProxyGetter(target, name, port, objectId) {
    ...
    let result = commonFieldGetter(name, objectId, nameStr);
    if (result) return result;

    logToFile(`get.1 ${nameStr}[${objectId}].'${String(name)}', querying type ...`);
    ...
    logToFile(`get.2 ${nameStr}[${objectId}].'${String(name)}' resulted in ${print(result)}`);
    return result;
}
