...
function rpcH2WCall(port, targetId, memberId, args, resultId, isAsync) {
    ...
    logToFile(`RunOnPaused after, callResult=${print(callResult)}`);

    // TODO: proper deserialization
    const proxyTag = getValueIfPresent(callResult, IDS.COMM.PROXY);
    if (proxyTag) {
        const resultToBe = context.cache.lookUpItem(proxyTag);
        logToFile(`>>> ${proxyTag} -> resultToBe=${print(resultToBe)}`);
        return resultToBe;
    }
