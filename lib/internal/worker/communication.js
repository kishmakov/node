...
function rpcH2WCall(port, targetId, memberId, args, resultId, isAsync) {
    if (!IsThreadPaused(context.destId)) {
    ...
    logToFile(`RunOnPaused before targetId=${targetId} memberId=${memberId} args=${strArgs} resultId=${resultId}`);
    RunOnPaused(context.destId, targetId, memberId, strArgs, resultId, isAsync);
