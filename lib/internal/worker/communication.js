...
function asyncW2HType(port, targetId, serialize = false) {
    assert(typeof targetId === 'string', `W: asyncType called with nonstring targetId`);
    ...
    const typeResult = WaitType(context.threadId, targetId, callId, serialize);
...
function v8H2WCall(targetId, memberId, args, isAsync) {
    ...
    const callId = newCallId();
    logToFile(`RunOnPaused before targetId=${targetId} memberId=${memberId} args=${print(args)} callId=${callId}`);
    const result = RunOnPaused(context.threadId, context.destId, callId, H2W_CALL, targetId, memberId, args, isAsync);
