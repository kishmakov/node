...
function asyncW2HType(port, targetId, callId, serialize = false) {
    assert(typeof callId === 'string', `W: asyncType callId must be string`);
    ...
    if (context.getPaused()) {
        logToFile(`asyncW2HType delegates to v8H2WType ${coord}`)
        typeResult = v8H2WType(callId, targetId, serialize);
    } else {
        sendMessage(port, W2H_TYPE, {targetId, callId, serialize, threadId: context.threadId});
        logToFile(`W: WaitType before ${coord}`);
        typeResult = WaitType(context.destId, callId, targetId, serialize);
...
function postW2HCall(port, callId, targetId, memberId, args, resultId) {
    assert(typeof targetId === 'string', `failed to supply string targetId to postW2HCall`);
    assert(!memberId || typeof memberId === 'string', `failed to supply string memberId to postW2HCall`);
...
function v8H2WCall(targetId, memberId, args, isAsync, resultId) {
    ...
    const result = RunOnPaused(context.destId, callId, H2W_CALL, targetId, memberId, args, isAsync, false, resultId);
    logToFile(`RunOnPaused after callId=${callId} result=${print(result)}`);
    return result;
}

function v8H2WType(callId, targetId, serialise) {
    assert(typeof targetId === 'string', `v8H2WType called with non-string targetId`);
    logToFile(`RunOnPaused before callId=${callId} targetId=${targetId}`);
    const result = RunOnPaused(context.destId, callId, H2W_TYPE, targetId, null, null, false, serialise, null);
    logToFile(`RunOnPaused after callId=${callId} result=${print(result)}`);
    return result;
}
