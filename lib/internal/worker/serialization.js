...
function serializeObject(object, objectId) {
    ...
    while (current && current !== Object.prototype) {
        ...
        for (const memberId of Reflect.ownKeys(current)) {
            ...
            result[memberId] = serializeArgument(value, objectId, String(memberId));
        }
...
function serializeArgument(arg, targetId = null, memberId = null) {
    const tracker = context.getTracker(arg);
    if (tracker) return tracker;

    const proxyTag = getValueIfPresent(arg, context.proxyTagKey())
    if (proxyTag) return {[IDS.REPORTED]: proxyTag};
    ...
    logToFile(`serializeArgument(${print(arg)}) -> ${print(result, false)}`);
    return result;
}
