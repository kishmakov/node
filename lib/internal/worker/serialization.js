...
function createTracker(value, itemId) {
    assert(itemId, `createTracker: input failed: got itemId=${itemId}`);
    context.cache.storeItem(itemId, value);
    const tracker = {[IDS.COMM.ITEM]: itemId};
    tracker[IDS.COMM.JSON] = valueToJson(value, itemId);
    return tracker;
}

function getOrCreateTracker(value, externalId = null) {
    let tracker = context.getTracker(value);
    if (tracker) return tracker;

    externalId = externalId || context.newId();
    tracker = createTracker(value, externalId);
...
function serializeValue(value, externalId = null) {
    const proxyId = getValueIfPresent(value, IDS.TAGS.PROXY_ITEM)
    return proxyId ? {[IDS.COMM.PROXY]: proxyId} : getOrCreateTracker(value, externalId);
}

function serializeResult(callId, value) {
    const handle = serializeValue(value, callId);
