...
function serializeResult(resultValue, resultId, provideJson = false) {
    ...
    if (isFunction(resultValue)) resultValue.isClass = isClass(resultValue);
    ...
    if (provideJson) {
        resultSer[IDS.COMM.JSON] = safeStringify(serializeItem(resultValue));
    }
...
function setSerializationContext(threadContext) {
    ...
    assert(isFunction(context.proxyTagKey), 'context.proxyTagKey must be present');
