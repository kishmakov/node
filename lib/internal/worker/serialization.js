...
function safeStringify(value) {
    try {
            ...
            if (val && typeof val === 'object') {
                ...
                for (const key of Reflect.ownKeys(val)) {
                    try {
                        ...
                        obj[key] = {$error: err.message || String(err)};
                    }
                }
        ...
        try {
            const result = JSON.stringify(helper(value));
            logToFile(`>>> ${print(result, false)}`);
            return result;
        } catch (e2) {
...
function safeParse(jsonStr) {
    ...
    const hasMarkers = (function checkMarkers(root) {
        ...
        while (stack.length > 0) {
            ...
            if ('$ref' in val || ('$id' in val && '$value' in val)) return true;
    ...
    function build(node) {
        ...
        if (Object.prototype.hasOwnProperty.call(node, '$ref')) {
            return byId.get(node.$ref);
        }

        // freshly defined object/array with id
