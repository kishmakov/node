...
function serializeArguments(port, postW2HUpdate, args) {
    ...
    return context.isThreadPaused() ? JSON.stringify(result) : result;
}
...
function serializeResult2(value, resultId, serialize) {
    const resultSer = {
        [IDS.COMM.RESULT]: resultId,
        [IDS.COMM.VALUE]: value,
    };

    if (serialize) {
        try {
            resultSer[IDS.COMM.JSON] = JSON.stringify(serializeArgumentPost(value));
        } catch (e) {
            logToFile(`>>> serializeResult failed for ${print(value)}:\n>>> error=${e}`, true);
        }
    }
