...
let context = null;
function setCacheContext(threadContext) { context = threadContext; }
function installCache(threadContext) {
    threadContext.cache = threadContext.isOnHost ? hostCache : workerCache;
    logToFile(`Installed cache to context ${threadContext.threadId}`);
}
