...
const {anyPromise} = require("internal/worker/communication");
...
function valueToJson(value) {
    ...
    function jsonify(val) {
        ...
        if (type === EntityType.OBJECT) {
            const obj = {};
            for (const key of Reflect.ownKeys(val)) obj[key] = jsonify(val[key]);
            result[IDS.SER.VALUE] = obj;
        }

        if (type === EntityType.ARRAY) {
            const arr = {};
            for (let i = 0; i < val.length; i++) arr[i] = jsonify(val[i]);
        ...
        if (type === EntityType.PROMISE) {
            val
                .then(res => anyPromise(context.port, id, true, res))
                .catch(err => anyPromise(context.port, id, false, err));
        }
    ...
    try {
        ...
        logToFile(`jsonify failed: ${e.message}`, true);
        return null;
    }
}
...
function jsonToAssembler(json) {
    ...
    const slim_types = new Set([EntityType.PROMISE, EntityType.FUNCTION, EntityType.ASYNC_FUNCTION, EntityType.CLASS]);
