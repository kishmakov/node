...
// const {anyCall, anySet} = require("internal/worker/communication");
...
function createGetter(port, target, memberId, proxyStr) {
    if (memberId in target) return target[memberId];
    const value = commonFieldGetter(target, memberId, proxyStr);
...
function createHandler(port, id, proxyStr) {
    ...
    return {
        __proto__: null,
        get: (target, memberId) => createGetter(port, target, memberId, proxyStr),
