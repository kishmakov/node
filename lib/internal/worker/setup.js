...
const {setCommunicationContext, setupCommunicationFunctionality} = require("internal/worker/communication");
const {setupProxyFunctionality} = require("internal/worker/proxy");
const {setSerializationContext, setupSerializationFunctionality} = require("internal/worker/serialization");
...
function setupEnvironment(port, threadId, destId) {
    ...
    if (threadId === IDS.HOST) {
        ...
        setAPI(`h:promise.${destId}`, 'used'); // TODO: adapt for multiple extensions
...
function prepareHost(port, wd) {
    ...
    const beenhere1 = getAPI('BEENHERE1');
    const beenhere2 = getAPI('BEENHERE2');

    console.log(`>>> BEENHERE1=${beenhere1}`);
    console.log(`>>> BEENHERE2=${beenhere2}`);
    if (getAPI(`h:promise.${wd.id}`) === 'initial') return false;
    setupEnvironment(port, IDS.HOST, wd.id);
    if (contexts.size > 1) logToFile(`---- context is set for destId=${wd.id} ----`);
    return true;
}
