...
const {
  initWorker,
  reactOnHostMessage,
} = require('internal/worker/extension');

const { logToFile } = require('internal/worker/utils');
...
port.on('message', (message) => {
  try {
    if (reactOnHostMessage(message)) {
      return;
    }
  } catch (e) {
    const msg = JSON.stringify(message);
    logToFile(`W: error during handling ${msg}\n:W: ${e}\nW: stack: ${e.stack}`)
    return;
  }
  ...
  if (message.type === LOAD_SCRIPT) {
    ...
    if (workerData && !!workerData.id) {
      initWorker(workerData.id, publicPort);
      logToFile(`W: Received ${LOAD_SCRIPT}: ${filename}`);
    }
