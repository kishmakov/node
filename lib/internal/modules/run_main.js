...
const { getAPI, kEmptyObject, getCWDURL } = require('internal/util');
const { resolveActivationResult } = require('internal/worker/extension');
const {
...
function executeUserEntryPoint(main = process.argv[1]) {
  ...
  if (useESMLoader) {
    ...
    const { Module } = require('internal/modules/cjs/loader');
    const module = Module._load(main, null, true);
    if (typeof module.activate === 'function') {
      const context = getAPI('w:context');
      const res = module.activate.apply(global, [context]);
      resolveActivationResult(res);
    }
  }
}
