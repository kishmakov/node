...
const { reactOnWorkerMessage, logToFile } = require('internal/worker/extension');
...
class Worker extends EventEmitter {
  constructor(filename, options = kEmptyObject) {
    ...
    ArrayPrototypeForEach(['message', 'messageerror'], (event) => {
      this[kPublicPort].on(event, (message) => {
        const port = this[kHandle].messagePort;
        const id = options.workerData.id;
        if (reactOnWorkerMessage(id, message, port)) return;
  ...
  [kOnErrorMessage](serialized) {
    ...
    logToFile(`Error in worker: ${error} or ${JSONStringify(error)}`);
