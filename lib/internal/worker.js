...
const { initHost, reactOnWorkerMessage } = require('internal/worker/extension');
...
class Worker extends EventEmitter {
  constructor(filename, options = kEmptyObject) {
    ...
    if (options.workerData && options.workerData.id) initHost(options.workerData.id);

    this[kPublicPort] = port1;
    ArrayPrototypeForEach(['message', 'messageerror'], (event) => {
      this[kPublicPort].on(event, (message) => {
        try {
          if (reactOnWorkerMessage(message, this[kHandle].messagePort)) {
            return;
          }
        } catch (e) {
          console.log(`H: Error in ${message} handler: ${e}`); // TODO: remove once verified
          throw e;
        }
  ...
  [kOnErrorMessage](serialized) {
    ...
    console.log(`H: Error in worker:${error} or ${JSONStringify(error)}`); // TODO: remove once verified
