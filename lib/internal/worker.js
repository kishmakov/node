...
const {logToHostLog, reactOnMessage} = require('internal/worker/extension');
const {prepareHost} = require('internal/worker/setup');
    ...
    ArrayPrototypeForEach(['message', 'messageerror'], (event) => {
      this[kPublicPort].on(event, (message) => {
        if (options.workerData.id) {
          prepareHost(this[kHandle].messagePort, options.workerData.id);
