...
const {reactOnMessage} = require('internal/worker/extension');
const {logToHostLog, prepareHost} = require('internal/worker/setup');
...
class Worker extends EventEmitter {
  constructor(filename, options = kEmptyObject) {
    ...
    ArrayPrototypeForEach(['message', 'messageerror'], (event) => {
      this[kPublicPort].on(event, (message) => {
        if (options.workerData.id) {
          console.log(`[${new Date().toISOString()}] >>> Host received message:`, message);
          if (reactOnMessage(message)) return;
        }
        this.emit(event, message);
      });
    });
    if (options.workerData) {
      prepareHost(this[kPort], options.workerData);
    }
  ...
  [kOnErrorMessage](serialized) {
    ...
    logToHostLog(`Error in worker: ${error} or ${JSONStringify(error)}`);
